...
# Corrige UI: adicionar/remover opera no array em memória e redesenha sem recarregar SharedPreferences.
...
# separar load de render para não perder item recém-adicionado
...
helper=r'''
    private boolean isGroupNotification(Notification n, Bundle extras) {
        try {
            ...
            if (conv != null && conv.toString().trim().length() > 0) return true;
            ...
            if (android.os.Build.VERSION.SDK_INT >= 24 && n.getNotificationStyle() != null) {
                // MessagingStyle com conversation title é grupo; conv acima é a fonte principal.
            }
...
print('v2.1.15: privado isolado de grupos; grupos usam somente menu; adicionar opção corrigido')